...
function UserList({
  users, activeUsers, activeUid, setVisible, visible, sorter, setSorter,
}:{
    activeUsers: string[],
    users: UsersObj,
    activeUid:string,
  ...
  return (
    <ul className="unlisted userListAdmin">
      ...
      {activeUsers.map((uid) => {
        ...
        return (
          <li key={uid}>
            <Link className="linkToProfile" to={`/admin/perfiles/${uid}`}>
...
export default function PerfilesAdmin() {
  ...
  if (!currentUid) currentUid = activeUids[0]; // we want to get also the empty strings
        ...
        : (
          <div>
            <Perfil
              user={currentUser}
              isAdmin
              setFn={(val:string|boolean, path:string) => setDataOfUser(currentUid, val, path)}
            />
          </div>
        )}
      <UserList
